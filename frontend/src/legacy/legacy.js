...
async function loadDisasterFeed() {
  ...
  try {
    ...
    if (!items.length) {
      listEl.innerHTML = '<div style="padding:14px 4px;color:var(--text-soft);font-size:13px;">No active natural events or significant earthquakes reported for this region right now.</div>';
      return;
    }
...
async function openIncidentModal(incidentId) {
  ...
  try {
    ...
    const photo = incident.photoUrl
      ? `<img src="${escapeHtml(fileUrl(incident.photoUrl))}" alt="Incident evidence" style="width:100%;border-radius:10px;margin-top:10px;max-height:260px;object-fit:cover;">`
      : '';
    body.innerHTML =
...
async function loadAlertsPageData(root) {
  ...
  wireAlertItemClicks(listEl);
  try {
    const { alerts, unreadCount } = await alertApi.list(30);
