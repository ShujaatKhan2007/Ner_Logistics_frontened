...
import { t, getLanguage, setLanguage, applyTranslations, initLanguage, onLanguageChange } from '../i18n/i18n.js';
...
export function installGlobalBindings() {
  ...
  onLanguageChange(() => {
    // applyTranslations() (called by setLanguage) already handles every
    // data-i18n element. This refreshes the handful of things rendered as
    // JS template strings (alert tags, time-ago labels, the dashboard
    // greeting) so they don't stay stuck in the old language until the
    // user navigates away and back.
    renderDashboardGreeting();
    loadAlertsPageData(document);
  });
}
...
async function handleRegisterSubmit(role) {
  ...
  const professionSelect = panel.querySelector('select');
...
function timeAgo(dateStr) {
  ...
  if (mins < 60) return t('time_min_ago').replace('{n}', mins);
  const hrs = Math.floor(mins / 60);
  if (hrs < 24) return t('time_hr_ago').replace('{n}', hrs);
  return t('time_day_ago').replace('{n}', Math.floor(hrs / 24));
}
...
async function updateUnreadBadge() {
  ...
  try {
    ...
    if (badge) badge.textContent = unreadCount > 0 ? t('common_unread').replace('{n}', unreadCount) : t('common_allCaughtUp');
  } catch {
    /* ignore */
  }
}
...
async function loadDashboardData(root) {
  ...
  if (!window.__nerGreetingInterval) {
    window.__nerGreetingInterval = setInterval(renderDashboardGreeting, 5 * 60 * 1000);
  }
...
function wireAlertsFilter(root) {
  ...
  menu.querySelectorAll('.alerts-filter-option').forEach((opt) => {
    opt.addEventListener('click', (e) => {
      ...
      if (label) label.textContent = `Filter: ${opt.textContent}`;
      menu.classList.remove('open');
...
async function loadAlertsPageData(root) {
  ...
  try {
    ...
    renderFilteredAlerts();
    const badge = document.getElementById('alertsUnreadBadge');
    if (badge) badge.textContent = unreadCount > 0 ? `${unreadCount} unread` : 'All caught up';
  } catch (err) {
